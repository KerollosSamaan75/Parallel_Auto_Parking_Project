...
static Std_ReturnType lcd_send_4bits(const chr_lcd_4bit_t *lcd, uint8 _data_command){
    Std_ReturnType ret = E_OK;
    HAL_GPIO_WritePin(lcd->lcd_data[0].port, lcd->lcd_data[0].pin, (_data_command >> 0) & (uint8)0x01);
    HAL_GPIO_WritePin(lcd->lcd_data[1].port, lcd->lcd_data[1].pin, (_data_command >> 1) & (uint8)0x01);
    HAL_GPIO_WritePin(lcd->lcd_data[2].port, lcd->lcd_data[2].pin, (_data_command >> 2) & (uint8)0x01);
    HAL_GPIO_WritePin(lcd->lcd_data[3].port, lcd->lcd_data[3].pin, (_data_command >> 3) & (uint8)0x01);
    return ret;
}
